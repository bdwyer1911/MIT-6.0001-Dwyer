...
"""
I made a few changes to the homework code to make it better:
1. upped the guesses from 6 to 9
2. removed a bad guess of a vowel counting as 2 guesses. That's dumb
3. for hangman with hints, I changed the possible matches to only be words
that did not include the letters already guessed. So it would only print 
out actual words that could be the guesses word.
"""
...
def hangman(secret_word):
    ...
    * After each guess, you should display to the user the 
    ...
    guesses_remaining = 9 #initialize the number of guesses
...
def match_with_gaps(my_word, other_word, guessed):
    '''
    ...
    unspaced = my_word.replace(" ", "")
    if len(unspaced) == len(other_word):
        for i in range(len(unspaced)):
            if unspaced[i] == other_word[i]:
                value = True
            elif unspaced[i] == '_' and other_word[i] not in unspaced and other_word[i] not in guessed:
                value = True
            else:
                value =  False
                break
...
def show_possible_matches(my_word, guessed):
    '''
    my_word: string with _ characters, current guess of secret word
    ...
    for words in wordlist:
        if match_with_gaps(my_word, words, guessed):
...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while not is_word_guessed(secret_word, letters_guessed) and guesses_remaining > 0:
        ...
        #check if valid guess
        elif guess not in string.ascii_lowercase or guess in letters_guessed:
            warnings_remaining -= 1
